...
mainstm_est_naive = redd_results %>%
  filter(str_detect(Reach,
                    "MRW[[:digit:]]",
                    negate = F)) %>%
  mutate(Location = "Mainstem Methow") %>%
...
trib_spwn <- trib_spawners %>%
  ...
  rowwise() %>%
  mutate(Org_Spawners = Spawners * prop,
         Org_SE = deltamethod(~ x1 * x2,
                              mean = c(Spawners, prop),
...
To convert estimates of redds in mainstem areas into estimates of natural and hatchery spawners, the estimates of redds were multiplied by a fish per redd (FpR) estimate and then by the proportion of hatchery or wild fish. The fish per redd estimate was based on PIT tags from the branching patch-occupancy model observed to move into the mainstem Methow, but not into tributaries. FpR was calculated as the ratio of male to female fish, plus 1. Reaches MRW1 - MRW8 are all in the mainstem Methow below Winthrop. Similarly, the proportion of hatchery and natural origin fish was calculated from the same group of PIT tags for areas in the mainstem, and the various tributaries. 
...
summ_redd_tab %>%
  ...
  kable(digits = c(rep(1, 5), 1, 3),
        booktabs = T,
        format.args = list(drop0trailing = T),
        caption = "Estimate of redds in mainstem Methow, and any tributaries surveyed.") %>%
  kable_styling(bootstrap_options = c("striped", 
                                      "hover"),
                latex_options = c("striped",
                                  "scale_down")) %>%
...
Parameter estimates for fish / redd in the lower Methow are shown in Table \@ref(tab:fpr-prop). For the four main areas of the Methow, the number of wild, HOR-SN (hatchery origin safety net) and HOR-C (hatchery origin conservation) tags and their proporitions are shown in Table \@ref(tab:hatch-prop). 
...
met_prop_origin %>%
  kable(digits = 3,
        booktabs = T,
        caption = "Tags and proportion of each origin group, by conservation area.",
...
ps_mort_df %>%
  ...
  kable(digits = 3,
        booktabs = T,
        caption = "Mainstem Methow prespawn mortality estimates. Includes estimates (standard error) of escapement, spawners, prespawn mortality, and the standard error of this rate, separated by origin and mainstem areas below Winthrop.") %>%
  kable_styling(bootstrap_options = c("striped", "hover"),
                latex_options = "striped")
```
...
Estimates of prespawn mortality were lower for the Methow overall compared to only the mainstem areas (TableS \@ref(tab:ps-mort-met) and \@ref(tab:ps-mort-main)). This reflects our assumption that steelhead may overwinter in the mainstem Methow for a considerable period of time, but when they move into the tributaries they are ready to spawn and therefore PIT tag estimates of escapement to those tributaries are also estimates of spawners in those tributaries. 

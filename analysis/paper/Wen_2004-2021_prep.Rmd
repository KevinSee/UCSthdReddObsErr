...
spwn_org_redd_pit <- all_redds %>%
  filter(method != "Index",
         between(year, 2011, 2013)) %>%
  group_by(year,
           river,
           location,
           method) %>%
  ...
  left_join(fpr_df %>%
              select(year,
                     ...
                     phos_all = phos,
                     phos_all_se = phos_se)) %>%
  mutate(fpr = if_else(is.na(fpr),
                       fpr_all,
                       fpr),
         fpr_se = if_else(is.na(fpr_se),
                       fpr_all_se,
                       fpr_se),
         phos = if_else(is.na(phos),
                       phos_all,
                       phos),
         phos_se = if_else(is.na(phos_se),
                       phos_all_se,
                       phos_se)) %>%
...
spwn_org_redd %>%
  anti_join(spwn_org_redd_pit,
            by = c("year",
                   "river",
                   "location",
                   "n_rch")) %>%
  bind_rows(spwn_org_redd_pit) %>%
  arrange(year,
          river,
          location) -> spwn_org_redd
...
We then expanded the estimates of spawners in the total Wenatchee by this proportion, to account for the fact that no redd surveys were conducted in those tributaries, and so any steelhead spawners there were missing from estimates in 2004-2010. We did not expand estimates for 2011-2013, because we did have PIT tag estimates of escpement to those tributaries for those years. 

...
ps_mort_df = escp_wen %>%
  ...
  left_join(spwn_rch %>%
              select(-ends_with('cv')) %>%
              pivot_longer(Hatchery:Natural_SE,
                           names_to = "type",
                           values_to = "value") %>%
              mutate(Origin = str_remove(type, '_SE'),
                     across(type,
                            ~ if_else(str_detect(type, "_SE$"),
                                      "spwn_se",
                                      "spwn_est"))) %>%
...
The data for this report was collected by Washington Department of Fish and Wildlife and the Chelan County Public Utility District.  Development of the observer error model was done in collaboration with Andrew Murdoch, WDFW.

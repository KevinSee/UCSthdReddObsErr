...
redd_results = redd_df %>%
  ...
  mutate(GAUC = if_else(!converged | n_non0_wks < min_non0_wks | tot_feat < min_redds,
                        F, T)) %>%
  mutate(GAUC = if_else(GAUC & is.na(redd_est),
                        F, GAUC)) %>%
  rowwise() %>%
...
spwn_rch = redd_results %>%
  ...
  mutate(Location = if_else(Reach == "C1",
                            "Chiwawa",
                            if_else(Reach == "N1",
                                    "Nason",
                                    if_else(Reach == "P1",
                                            "Peshastin",
                                            if_else(!grepl('W[[:digit:]]', Reach),
                                                    'Tribs_above_TUM',
                                                    if_else(Reach %in% paste0('W', 1:7), 
                                                            'Below_TUM', 
                                                            'TUM_bb')))))) %>%
...
Estimates of escapement to various tributaries in the Wenatchee were made using a branching patch-occupancy model [@Waterhouse2020] based on PIT tag observations of fish tagged at Priest Rapids dam. All fish that escaped to the various tributaries were assumed to be spawners (i.e. prespawn mortality only occurs in the mainstem). Reaches below the PIT tag arrays in some tributaries were surveyed for redds, but we assumed there was no observer error in those reaches. 
...
The estimates of overall prespawn mortality within the Wenatchee population, by origin, are shown in Table \@ref(tab:ps-mort-wen). The estimates of prespawn mortality in mainstem areas above and below Tumwater dam are displayed in Table \@ref(tab:ps-mort-tum). 
...
ps_mort_df %>%
  ...
  kable(digits = 3,
        booktabs = T,
        caption = "Mainstem Wenatchee prespawn mortality estimates. Includes estimates (standard error) of escapement, spawners, prespawn mortality, and the standard error of this rate, separated by origin and mainstem areas above and below Tumwater dam.") %>%
  kable_styling(bootstrap_options = c("striped", "hover"),
                latex_options = "striped")
```
...
All the estimates of spawners across various spatial areas were smaller than (or equal to) the estimates of escapement, which provided legitimate estimates of prespawn mortality. Overall, the estimates of prespawn mortality were smaller for natural origin fish compared to hatchery fish, whether calculated for the entire Wenatchee mainstem, or splitting it into areas above and below Tumwater dam. 

...
Esimates of escapement to various tributaries in the Wenatchee were made using a branching patch-occupancy model (Waterhouse, L. et al., *in review*) based on PIT tag observations of fish tagged at Priest Rapids dam. All fish that escaped to the various tributaries were assumed to be spawners (i.e. pre-spawn mortality only occurs in the mainstem).
...
ps_mort_wen %>%
  ...
  arrange(desc(Origin)) %>%
  kable(digits = 3,
        caption = "Wenatchee pre-spawn mortality estimates. Includes estimates (standard error) of escapement, spawners, pre-spawn mortality, and standard error of this rate, separated by origin.")
```

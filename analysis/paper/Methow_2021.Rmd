...
rem_df <- tibble(Source = c("Mainstem broodstock",
                            "Methow hatchery",
                            "WNFH hatchery",
                            "Twisp weir")) %>%
  crossing(Origin = c("Hatchery",
                      "Natural")) %>%
  mutate(rem = 0) %>%
  mutate(Area = if_else(Source == "Twisp weir",
                        "Twisp",
                        "Mainstem Methow"))
...
spwn_rch = redd_results %>%
  ...
  bind_rows(trib_spawners %>%
              ...
              mutate(Location = recode(Location,
                                       ...
                                       'TWR' = 'Twisp')) %>%
              mutate(Type = 'Trib')) %>%
  mutate(Location = factor(Location, levels = c('Mainstem Methow',
                                                'Gold', 
                                                'Libby', 
                                                'Beaver', 
                                                'Spring Creek', 
  ...
  left_join(spwn_rch %>%
              ...
              mutate(Origin = str_remove(type, '_SE'),
                     type = if_else(grepl("_SE$", type),
                                    ...
                                    "spwn_est")) %>%
              spread(type, value) %>%
              filter(Location %in% c('Mainstem Methow')) %>%
...
met_sex_tab %>%
  ...
  left_join(met_origin_tab %>%
              select(Location, h_prop, prop_se)) %>%
  filter(Location != 'Tribs_above_MRC') %>%
  mutate(Location = recode(Location, 
                           'AboveWinthrop' = 'Above Winthrop')) %>%
  kable(digits = 3,
        booktabs = T,

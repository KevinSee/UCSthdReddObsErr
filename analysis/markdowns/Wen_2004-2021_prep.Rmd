...
subtitle: "Spawn Years 2004-2021"
author:
  - Kevin See:
...
census_redds <- census_surv %>%
  mutate(location = if_else(!str_detect(index_reach, 'W[[:digit:]]'),
                            river,
                            if_else(index_reach %in% paste0('W', 1:7),
                                    'Below_TUM',
                                    'Above_TUM'))) %>%
...
```{r spawner-origin}
spwn_org_redd_river_fpr <- all_redds %>%
  select(year, 
         river,
         location,
         method,
...
#                      phos),
#                    mean)) %>%
#   select(any_of(names(spwn_org_redd_river_fpr)))
...
From 2011 onwards, we used the DABOM model to estimate tributary spawners. We assumed that every fish that escaped into a tributary was a successful spawner. Because counts at Priest Rapids dam have been questionable in recent years, we used DABOM estimates calculated based on counts at Rock Island dam for 2016, 2017, 2019, 2020 and 2021 (estimates in 2018 were very similar regardless of whether Priest or Rock Island counts were used, so we used Priest that year).
  ...
  mutate(pit_info = map2(year,
                   ...
                   .f = function(yr,
                     ...
                     load(paste0('O:Documents/Git/MyProjects/DabomPriestRapidsSthd/analysis/data/derived_data/estimates/', dam_cnt_name,
                                 "/old/PRA_DABOM_Steelhead_", yr, ".rda"))
...
age_comp_org = pit_list %>%
  mutate(tags = map(pit_info,
                    "tags")) %>%
  select(-pit_info) %>%
  unnest(tags) %>%
  left_join(read_excel(here("T:/DFW-Team FP Upper Columbia Escapement - General/UC_Sthd/estimates",
                            "UC STHD_Model_Output_Examp.xlsx"),
                       "Age_Cheat_Sheet",
                       skip = 1) %>%
              clean_names() %>%
              rename(run_sh = x3,
  ...
  group_by(year, 
           origin = Origin) %>%
  count(total_age) %>%
  mutate(total_aged = sum(n)) %>%
  mutate(n = n / total_aged) %>%
  ungroup() %>%
  pivot_wider(names_from = total_age,
              values_from = n,
              names_sort = T,
              values_fill = 0) %>%
...
```{r}
# spwn_org_est <- spwn_org_redd_river_fpr %>%
...
We then expanded the estimates of spawners in the total Wenatchee for 2004-2010 by this proportion, to account for the fact that no redd surveys were conducted in those tributaries, and so any steelhead spawners there were missing from estimates in those years. We did not expand estimates for 2011-2013, because we did have PIT tag estimates of escapement to those tributaries for those years. 

This process also allowed us to compile the redd and spawner estimates from 2014-2021, which utilized the two-observer redd observer error model from @Murdoch2018 for the mainstem spawners (redd counts) and PIT-tag based estimates of escapement to all the tributaries. This includes some redds counted in tributaries downstream of the PIT tag arrays. For these, we assumed no observer error, and applied the fish/redd and pHOS estimate from that tributary based on observed PIT tags in that tributary.
...
```{r, eval = T}
# use fish/redd and pHOS estimates from the entire river based on Dryden sampling 
recent_spwn_redd_river_fpr <- all_wen_df %>%
  select(year,
         loc_est) %>%
  unnest(loc_est) %>%
  left_join(fpr_df %>%
...
recent_spwn_redd <- all_wen_df %>%
  select(year,
         loc_est) %>%
  unnest(loc_est) %>%
  full_join(all_wen_df %>%
              mutate(wen_sex_org = map(wen_sex_org,
                                       .f = function(x) {
                                         x %>%
                                           rename(phos = h_prop,
                                                  phos_se = prop_se) %>%
...
prop_trib <- trib_spawners_all %>%
  filter(between(year, 2011, max(recent_spwn_all$year)),
...
spwn_org_expnd <- spwn_org_est %>%
  ...
  mutate(across(hor_spwn,
                ~ if_else(!year %in% unique(trib_spawners_all$year),
                          . / (1 - hor_prop),
                          .)),
         across(nor_spwn,
                ~ if_else(!year %in% unique(trib_spawners_all$year),
                          ...
                          .))) %>%
  rowwise() %>%
  mutate(hor_se = if_else(!year %in% unique(trib_spawners_all$year),
                          deltamethod(~ x1 / (1 - x2),
                                      mean = c(hor_spwn,
                                               hor_prop),
                                      cov = diag(c(hor_se,
                                                   hor_prop_se)^2)),
                          hor_se),
         nor_se = if_else(!year %in% unique(trib_spawners_all$year),
                          deltamethod(~ x1 / (1 - x2),
                                      mean = c(nor_spwn,
                                               nor_prop),
                                      cov = diag(c(nor_se,
...
Table \@ref(tab:init-spwn-tab) shows the initial estimates, by origin, of spawners for spawn years 2004-2013. This only includes areas surveyed for redds or that had PIT tag escapement estimates (some tributaries in 2011-2013). Some of those areas had the redd surveys replaced by PIT tag escapement estimates in later years. 
...
Table \@ref(tab:expn-tab) shows the proportion of the overall number of spawners in 2014-2021 (except 2020) estimated to be in Chiwaukum, Chumstick and Mission creeks. These are the proportions used to expand older estimates (pre-2011) of total spawners by origin. 
...
age_comp_org %>%
  mutate(across(-c(year,
                ...
                digits = 4)) %>%
  write_csv(file = here("analysis/data/derived_data",
                      "Wenatchee_Sthd_Age_2011-2021.csv"))
...
```{r spwn-ts, fig.cap = "Time-series of spawner estimates, facted by origin. Colors show the estimation method. Error bars show the 95% confidence intervals."}
wen_spwn %>%
